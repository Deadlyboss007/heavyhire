        ...
        <div class="custom-shadow h-[100%] rounded-[20px]">
            ...
            <?php
                ...
                while($row = $run->fetch_assoc()){
                    ...
                    $check_already_booked = "select * from book where user_id=$user_id and driver_id=$acc_id";
                    $run_check = $con->query($check_already_booked);
                    $check_num = $run_check->num_rows;
...
<script>
    ...
    form.addEventListener("submit", (e) => {
        ...
        .then(response => response.json())
            .then(data => {
                window.location.replace('/heavyhire/client/user/profileDashboard.php')
            })
            .catch(error => {
                console.error(error);
            });
